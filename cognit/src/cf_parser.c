...
int8_t cfparser_parse_requirements_as_str_json(scheduling_t* t_app_requirements, uint8_t* ui8_payload_buff, size_t* payload_len)
{
    ...
    if (t_app_requirements->min_renewable < 100)
    {
...
int8_t srparser_parse_json_str_as_serverless_runtime(const char* json_str, serverless_runtime_t* t_serverless_runtime)
{
    ...
    //Name
    //  strcpy(t_app_requirements->c_name, cname_item->valuestring);

    //id
    COGNIT_LOG_DEBUG("%d%d", t_app_requirements->ui32_id, ui32_id_item->valueint);
    t_app_requirements->ui32_id = ui32_id_item->valueint;
    ...
    if (faas_config_item != NULL)
    {
        ...
        {
            t_app_requirements->faas_config.ui8_cpu = faas_cpu_item->valueint;
        }

        cJSON* faas_memory_item = cJSON_GetObjectItem(faas_config_item, "MEMORY");
        if (faas_memory_item != NULL)
        {
            t_app_requirements->faas_config.ui32_memory = faas_memory_item->valueint;
        }

        cJSON* faas_disk_size_item = cJSON_GetObjectItem(faas_config_item, "DISK_SIZE");
        if (faas_disk_size_item != NULL)
        {
            t_app_requirements->faas_config.ui32_disk_size = faas_disk_size_item->valueint;
        }

        cJSON* faas_flavour_item = cJSON_GetObjectItem(faas_config_item, "FLAVOUR");
        strcpy(t_app_requirements->faas_config.c_flavour, faas_flavour_item->valuestring);

        cJSON* faas_endpoint_item = cJSON_GetObjectItem(faas_config_item, "ENDPOINT");
        if (faas_endpoint_item != NULL)
        {
            strcpy(t_app_requirements->faas_config.c_endpoint, faas_endpoint_item->valuestring);
            COGNIT_LOG_DEBUG("t_app_requirements->faas_config.c_endpoint: %s", t_app_requirements->faas_config.c_endpoint);
        }

        cJSON* faas_state_item = cJSON_GetObjectItem(faas_config_item, "STATE");
        strcpy(t_app_requirements->faas_config.c_state, faas_state_item->valuestring);

        cJSON* faas_vm_id_item = cJSON_GetObjectItem(faas_config_item, "VM_ID");
        if (faas_vm_id_item != NULL)
        {
            t_app_requirements->faas_config.ui32_vm_id = faas_vm_id_item->valueint;
        }
    }
    else
    {
    ...
    if (daas_config_item != NULL)
    {
        ...
        {
            t_app_requirements->daas_config.ui8_cpu = daas_cpu_item->valueint;
        }
        cJSON* daas_memory_item = cJSON_GetObjectItem(daas_config_item, "MEMORY");
        if (daas_memory_item != NULL)
        {
            t_app_requirements->daas_config.ui32_memory = daas_memory_item->valueint;
        }
        ...
        {
            char* end;
            t_app_requirements->daas_config.ui32_disk_size = (uint32_t)strtoul(daas_disk_size_item->valuestring, &end, 10);
            // t_app_requirements->daas_config.ui32_disk_size = daas_disk_size_item->valuestring;
        }

        cJSON* daas_flavour_item = cJSON_GetObjectItem(daas_config_item, "FLAVOUR");
        strcpy(t_app_requirements->daas_config.c_flavour, daas_flavour_item->valuestring);

        cJSON* daas_endpoint_item = cJSON_GetObjectItem(daas_config_item, "ENDPOINT");
        if (daas_endpoint_item != NULL)
        {
            strcpy(t_app_requirements->daas_config.c_endpoint, daas_endpoint_item->valuestring);
        }

        cJSON* daas_state_item = cJSON_GetObjectItem(daas_config_item, "STATE");
        strcpy(t_app_requirements->daas_config.c_state, daas_state_item->valuestring);

        cJSON* daas_vm_id_item = cJSON_GetObjectItem(daas_config_item, "VM_ID");
        if (daas_vm_id_item != NULL)
        {
            t_app_requirements->daas_config.ui32_vm_id = daas_vm_id_item->valueint;
        }
    }
    ...
    if (scheduling_config_item != NULL)
    {
        ...
        strcpy(t_app_requirements->scheduling_config.c_policy, scheduling_policy_item->valuestring);

        cJSON* scheduling_req_item = cJSON_GetObjectItem(daas_config_item, "REQUIREMENTS");
        if (scheduling_req_item != NULL)
            strcpy(t_app_requirements->scheduling_config.c_requirements, scheduling_policy_item->valuestring);
    }
    ...
    if (device_info_item != NULL)
    {
        ...
        if (device_latency_item != NULL)
            t_app_requirements->device_info.ui32_latency_to_pe = device_latency_item->valueint;

        cJSON* device_loc_item = cJSON_GetObjectItem(device_info_item, "GEOGRAPHIC_LOCATION");
        strcpy(t_app_requirements->device_info.c_geograph_location, device_loc_item->valuestring);
    }
