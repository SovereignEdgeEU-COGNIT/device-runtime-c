...
// extern send_http_req_cb_t hhtp_send_req_cb;
...
send_http_req_cb_t http_send_req_cb(uint8_t* ui8_buffer, size_t size, http_config_t* config)
{
    CURL* curl;
    CURLcode res;
    ...
    if (curl)
    {
        ...
        else if (config->c_method == HTTP_METHOD_POST)
        {
            ...
            || curl_easy_setopt(curl, CURLOPT_POSTFIELDS, ui8_buffer) != CURLE_OK
